...
public class PhotonVisionFollow extends Command {
	...
	public PhotonVisionFollow(PhotonCamera photonCamera, DriveSubsystem drivetrainSubsystem,
	...
	public void execute() {
		...
		var robotPose = new Pose3d(robotPose2d.getX(), robotPose2d.getY(), 0.0,
		...
		if (photonRes.hasTargets()) {
			...
			var targetOpt =
					photonRes.getTargets().stream().filter(t -> t.getFiducialId() == TAG_TO_CHASE)
		...
		if (lastTarget == null) {
			...
			driveSubsystem.driveRobotRelative(ChassisSpeeds.fromFieldRelativeSpeeds(
					new ChassisSpeeds(xSpeed, ySpeed, omegaSpeed), robotPose2d.getRotation()));
		}
	}
